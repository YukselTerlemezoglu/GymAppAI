...
function CustomProgramBuilder({ setSavedAiProgram, setShowCustomBuilder }) {
    ...
    const [customDays, setCustomDays] = useLocalStorage('gym_app_custom_days', [{ dayName: '1. Gün - Göğüs & Arka Kol', exercises: [] }]);

    const handleAddCustomDay = () => {
        if (customDays.length >= 7) {
            alert("Bir programda en fazla 7 gün olabilir.");
            return;
        }
        setCustomDays([...customDays, { dayName: `${customDays.length + 1}. Gün`, exercises: [] }]);
    };
    ...
    const handleSaveCustomProgram = () => {
        if (!customProgramName.trim()) {
            alert("Lütfen program için bir isim girin (Örn: Push Pull Legs)");
            return;
        }
        const program = {
        ...
        <section className="custom-builder fade-in" style={{ animationDelay: '0.2s', marginBottom: '3rem' }}>
            <div className="section-header">
                <h2 className="section-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Dumbbell size={20} color="var(--accent-primary)" /> Özel Program Yarat
                </h2>
                <button onClick={() => setShowCustomBuilder(false)} className="clear-btn" aria-label="Kapat">
            ...
            <div className="glass-card" style={{ padding: '1.5rem', marginBottom: '2rem' }}>
                <div className="input-group">
                    ...
                    <input
                        ...
                        placeholder="Örn: 4 Günlük Arnold Split"
                    ...
                    <div key={dIdx} style={{ marginBottom: '1.5rem', background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '12px' }}>
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '1rem' }}>
                            <input
                                ...
                                placeholder="Gün Adı (Örn: Bacak Günü)"
                        ...
                        <button onClick={() => handleAddCustomExercise(dIdx)} className="neon-btn-secondary" style={{ fontSize: '0.8rem', padding: '0.5rem', width: '100%', marginTop: '0.5rem' }}>
                            <Plus size={14} /> Yeni Egzersiz Ekle
                        </button>
                    </div>
                ))}
